...
+ =luarocks, digestif=
  ...
  =sudo apt install luarocks liblua5.3-0 liblua5.3-dev= , =sudo luarocks --server http://luarocks.org/dev install digestif= 。用 =digestif= 作为默认的 =lsp-tex-server= ，可通过 =tex-server= 修改。
