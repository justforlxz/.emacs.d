...
(defun enable-flyspell ()
  "Enable command `flyspell-mode' if `flyspell' is not nil."
  (when (and flyspell (executable-find ispell-program-name))
    (flyspell-mode +1)))

(defun cleanup-maybe ()
  "Invoke `whitespace-cleanup' if `clean-whitespace-on-save' is not nil."
  (when clean-whitespace-on-save
    (whitespace-cleanup)))

(defun enable-whitespace ()
  "Enable `whitespace-mode' if `whitespace' is not nil."
  (when whitespace
    ;; keep the whitespace decent all the time (in this buffer)
    (add-hook 'before-save-hook 'cleanup-maybe nil t)
    (whitespace-mode +1)))
